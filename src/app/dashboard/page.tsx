...
export default function DashboardPage() {
  const { user, logout, cards, transactions } = useAuth();
        ...
        <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} transition-transform duration-300`}>
          <div className="flex flex-col h-full">
            ...
            <nav className="flex-1 p-4 space-y-1">
              ...
              <Link href="#" onClick={() => setActiveTab("transactions")} className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 font-medium">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" />
                </svg>
                Transactions
              </Link>
              <Link href="#" onClick={() => setActiveTab("settings")} className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-600 hover:bg-gray-50 font-medium">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="p-6 lg:p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                ...
                <div className="text-2xl font-extrabold text-gray-900">{cards.length}</div>
              ...
              <div className="bg-white rounded-2xl p-6 border border-gray-200 shadow-sm">
                ...
                <div className="text-2xl font-extrabold text-gray-900">{transactions.length}</div>
                    ...
                    {cards.length === 0 ? (
                      <div className="text-center py-8">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
                          <Link href="/checkout/virtual" className="px-6 py-3 bg-[#88D65E] text-black font-semibold rounded-xl hover:bg-[#76C14D] transition-colors">
                            Buy Virtual Card - $49
                          </Link>
                          <Link href="/checkout/physical" className="px-6 py-3 bg-gray-900 text-white font-semibold rounded-xl hover:bg-gray-800 transition-colors">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {cards.map((card) => (
                          <div key={card.id} className="relative bg-gradient-to-br from-gray-800 to-gray-900 rounded-2xl p-6 text-white">
                            {card.type === "physical" && (
                              <div className="absolute top-0 right-0 w-20 h-20 bg-gradient-to-br from-yellow-400/20 to-transparent rounded-full"></div>
                            )}
                            <div className="flex justify-between items-start mb-8">
                              <div className="w-10 h-6 bg-yellow-400 rounded-sm"></div>
                              <span className="text-xs font-bold">VISA</span>
                            </div>
                            <div className="text-xl font-mono tracking-widest mb-4">•••• •••• •••• {card.last4}</div>
                            ...
                            <div className="absolute bottom-6 right-6">
                              <span className={`px-2 py-1 text-xs font-medium rounded-full ${card.status === 'active' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}`}>
              ...
              <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
                <div className="p-6 lg:p-8">
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="py-4">
                      <button onClick={handleLogout} className="px-6 py-3 bg-red-50 text-red-600 font-medium rounded-xl hover:bg-red-100 transition-colors">
                        Sign Out
                      </button>
                    </div>
